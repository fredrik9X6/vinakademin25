...
const fragmentShader = `
  ...
  void main() {
    ...
    // Original settings from the component
    vec4 originalColor = cppn_fn(uv, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));
    ...
    // Define orange color palette (brand colors)
    vec3 orange = vec3(0.99, 0.73, 0.46); // #FDBA75
    vec3 darkOrange = vec3(0.98, 0.57, 0.30); // #FB914C
    vec3 veryDarkOrange = vec3(0.5, 0.25, 0.12); // Dark brown-orange
    
    // Theme-aware base color
    vec3 baseColor = isDarkMode > 0.5 
      ? vec3(0.09, 0.09, 0.09)  // Dark mode: #171717
      : vec3(1.0, 1.0, 1.0);     // Light mode: white
    
    // Invert gray for light mode so dark areas become light
    float adjustedGray = isDarkMode > 0.5 ? gray : 1.0 - gray;
    
    // Create gradient
    vec3 finalColor;
    if (adjustedGray < 0.35) {
      finalColor = mix(baseColor, veryDarkOrange, adjustedGray / 0.35);
    } else if (adjustedGray < 0.65) {
      finalColor = mix(veryDarkOrange, darkOrange, (adjustedGray - 0.35) / 0.3);
    } else {
      finalColor = mix(darkOrange, orange, (adjustedGray - 0.65) / 0.35);
    }
...
const CPPNShaderMaterial = shaderMaterial(
  { iTime: 0, iResolution: new THREE.Vector2(1, 1), isDarkMode: 1.0 },
  vertexShader,
  fragmentShader,
)
...
function ShaderPlane() {
  ...
  useFrame((state) => {
    ...
    materialRef.current.isDarkMode = isDark ? 1.0 : 0.0
  })
...
function ShaderBackground() {
  ...
  const camera = useMemo(
    () => ({ position: [0, 0, 5] as [number, number, number], fov: 75, near: 0.1, far: 1000 }),
    [],
  )
  ...
  useGSAP(
    ...
    { scope: canvasRef, dependencies: [isDarkMode] },
  )

  // Don't render shader until mounted or in light mode
  if (!isMounted || !isDarkMode) return null
...
export default function Hero({
  ...
  const microItem1Ref = useRef<HTMLDivElement | null>(null)
  const microItem2Ref = useRef<HTMLDivElement | null>(null)
  const microItem3Ref = useRef<HTMLDivElement | null>(null)
  ...
  useGSAP(
    () => {
      ...
      document.fonts.ready.then(() => {
        ...
        // Make h1 visible after split is ready
  ...
  return (
    <section ref={sectionRef} className="relative h-[calc(100vh-3.5rem)] w-full overflow-hidden bg-background">
      ...
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col items-center gap-7 px-6 pb-24 pt-20 text-center sm:gap-9 sm:pt-28 md:px-10 lg:px-16">
        ...
        <h1
          ref={headerRef}
          className="invisible text-balance text-4xl !leading-[1.15] text-foreground sm:text-5xl md:text-6xl lg:text-7xl"
        >
          {title}
        </h1>
