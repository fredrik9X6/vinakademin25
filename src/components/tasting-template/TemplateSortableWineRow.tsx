...
export interface TemplateSortableWineRowItem {
  key: string
  pourOrder: number
  title: string
  subtitle: string
...
export interface TemplateSortableWineRowProps {
  item: TemplateSortableWineRowItem
...
/**
 * Template-flavoured sortable row. Same big-faded-number-behind-bottle look
 * as `SortableWineRow` for plans, but with the simpler shape (library wines
 * only, no blind-answer inputs, no custom-wine fields).
 */
export function TemplateSortableWineRow({
  item,
  onNotesChange,
  onRemove,
  disabled,
}: TemplateSortableWineRowProps) {
