...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const fallbackTitle = `${country.name} | Vinakademin`
  const fallbackDescription = `Utforska viner och vinregioner från ${country.name}. Lär dig om landets vintraditioner och druvor.`
  const canonicalUrl = `${baseUrl}/lander/${slug}`
  const seo = resolveSeo(country as any, {
    title: fallbackTitle,
...
export default async function CountryDetailPage({ params }: PageProps) {
  ...
  // Fetch vinkurser that reference this country
